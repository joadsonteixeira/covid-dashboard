...
function main() {
    ...
    console.log("\n Começar regressão ");
    // 1. load data
    let trainX = U.loadTxt(".\\casos_confirmados.txt", "\t",
        [0, 1]);
    let trainY = U.loadTxt(".\\casos_confirmados.txt", "\t", [1]);
    let testX = U.loadTxt(".\\casos_confirmados.txt", "\t",
        [0, 1]);
    let testY = U.loadTxt(".\\casos_confirmados.txt", "\t", [1]);
    // 2. create network
    let seed = 0;
    let nn = new NeuralNet(2, 5, 1, seed);
    // 3. train network
    let lrnRate = 0.01;
    let maxEpochs = 5000;
    ...
    // nn.saveWeights(fn);
    // 6. use trained model
    let unknownRaw = [158556, 172684, 191788, 221801, 255175, 312533];
    let unknownNorm = [0.000471, 0.800000, 0.116569, -1, 0.014403, 0.488025, 0.299691, 0.735726, 0.000000, 0.244275, 0.404255, 0.989889, 0.133554];
    ...
    let predPrice = predicted[0] * 10;
    var casos_confirmados = parseInt(predPrice.toFixed(2));
    console.log("Casos confirmados no próximo mês: " + (parseInt(predPrice.toFixed(2))).toString());
    process.stdout.write("\033[0m"); // reset
    main2(casos_confirmados);
} // main()
main();
function main2(casos_confirmados) {
    process.stdout.write("\033[0m"); // reset
    process.stdout.write("\x1b[1m" + "\x1b[37m"); // bright white
    console.log("\n Começar regressão ");
    // 1. load data
    let trainX = U.loadTxt(".\\dataset.txt", "\t",
        [0, 1]);
    let trainY = U.loadTxt(".\\dataset.txt", "\t", [1]);
    let testX = U.loadTxt(".\\dataset.txt", "\t",
        [0, 1]);
    let testY = U.loadTxt(".\\dataset.txt", "\t", [1]);
    // 2. create network
    let seed = 0;
    let nn = new NeuralNet(2, 5, 1, seed);
    // 3. train network
    ...
    U.vecShow(predicted, 6, 1); // predicted is a vector.
    let predPrice = predicted[0] * 10;
    var obitos = parseInt(predPrice.toFixed(2));
    console.log("Casos confirmados no próximo mês: " + (parseInt(predPrice.toFixed(2))).toString());
    process.stdout.write("\033[0m"); // reset
    postBanco(casos_confirmados, obitos);
} // main()

function postBanco(casos_confirmados, obitos) {
    var data = JSON.stringify(
        {
            "casosPrevistos": casos_confirmados.toString(),
            "obitosPrevistos": obitos.toString()
        }
    );
    ...
    axios({
        ...
        data: {
          casosPrevistos: casos_confirmados,
          obitosPrevistos: obitos
        }
      });
}